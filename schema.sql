-- ============================================================================
-- INVENTORY REPORTING SYSTEM — COMPLETE SCHEMA SNAPSHOT
-- Pulled directly from the live Supabase database.
-- This is a SNAPSHOT as of the date below — re-pull after future changes.
-- See PROJECT_HANDOFF.md for full context on why things are built this way.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- TABLE STRUCTURE (reference only — tables already exist; this is not meant
-- to be re-run, just documents the current shape of each table)
-- ----------------------------------------------------------------------------
...
-- vw_stock_ledger:
--   store_code (text), barcode (text), sal_barcode (text), txn_date (date), in_qty (numeric), out_qty (numeric), source_type (text)

-- ----------------------------------------------------------------------------
-- VIEW: vw_stock_ledger
-- The unified ledger every stock calculation is built on top of.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- FUNCTIONS (in dependency order — helper functions first, report functions
-- that call them afterward)
-- ----------------------------------------------------------------------------
-- ---- fn_stock_qty ----------------------------------------------------------
CREATE OR REPLACE FUNCTION public.fn_stock_qty(as_of_date date, p_store_code text DEFAULT NULL::text)
...
-- ---- fn_pending_stock ------------------------------------------------------
CREATE OR REPLACE FUNCTION public.fn_pending_stock(as_of_date date, p_store_code text DEFAULT NULL::text)
...
AS $function$
    WITH delivered AS (
        ...
        WHERE txn_date <= as_of_date
          ...
          AND (p_store_code IS NULL OR delivery_to = p_store_code)
...
-- ---- fn_sale_barcode_stock_cycles ------------------------------------------
CREATE OR REPLACE FUNCTION public.fn_sale_barcode_stock_cycles(as_of_date date, p_store_code text DEFAULT NULL::text)
...
-- ---- fn_sale_barcode_stock_period ------------------------------------------
CREATE OR REPLACE FUNCTION public.fn_sale_barcode_stock_period(as_of_date date, p_store_code text DEFAULT NULL::text)
...
AS $function$
    ...
    FROM fn_sale_barcode_stock_cycles(as_of_date, p_store_code)
    GROUP BY store_code, barcode, sal_barcode
$function$
;

-- ---- fn_barcode_stock_age --------------------------------------------------
CREATE OR REPLACE FUNCTION public.fn_barcode_stock_age(as_of_date date, p_store_code text DEFAULT NULL::text)
...
-- ---- get_sale_barcode_wise_report ------------------------------------------
CREATE OR REPLACE FUNCTION public.get_sale_barcode_wise_report(as_of_date date, p_store_code text DEFAULT NULL::text, p_category text[] DEFAULT NULL::text[], p_sub_category text[] DEFAULT NULL::text[], p_search text DEFAULT NULL::text)
...
AS $function$
    WITH batch AS (
        ...
        FROM fn_stock_qty(as_of_date - 1, p_store_code) o
        FULL OUTER JOIN fn_stock_qty(as_of_date, p_store_code) h
            ON o.store_code = h.store_code AND o.barcode = h.barcode AND o.sal_barcode = h.sal_barcode
        FULL OUTER JOIN fn_pending_stock(as_of_date, p_store_code) p
    ...
    LEFT JOIN fn_sale_barcode_stock_period(as_of_date, p_store_code) sp
        ON sp.store_code = b.store_code AND sp.barcode = b.barcode AND sp.sal_barcode = b.sal_barcode
    WHERE (p_store_code IS NULL OR b.store_code = p_store_code)
...
CREATE OR REPLACE FUNCTION public.get_barcode_wise_report(as_of_date date, p_store_code text DEFAULT NULL::text, p_category text[] DEFAULT NULL::text[], p_sub_category text[] DEFAULT NULL::text[])
...
AS $function$
    WITH agg AS (
        ...
        FROM get_sale_barcode_wise_report(as_of_date, p_store_code, p_category, p_sub_category, NULL) r
    ...
    LEFT JOIN fn_barcode_stock_age(as_of_date, p_store_code) ba
        ON ba.store_code = s.store_code AND ba.barcode = a.barcode
    ORDER BY a.store_name, a.barcode
$function$
;

-- ---- get_store_wise_report -------------------------------------------------
CREATE OR REPLACE FUNCTION public.get_store_wise_report(as_of_date date, p_store_code text DEFAULT NULL::text, p_category text[] DEFAULT NULL::text[], p_sub_category text[] DEFAULT NULL::text[])
...
CREATE OR REPLACE FUNCTION public.get_distinct_categories()
 ...
 LANGUAGE sql
AS $function$
    SELECT DISTINCT category FROM product_file WHERE category IS NOT NULL ORDER BY category
$function$
;

-- ---- get_distinct_sub_categories -------------------------------------------
CREATE OR REPLACE FUNCTION public.get_distinct_sub_categories()
 RETURNS TABLE(sub_category text)
 LANGUAGE sql
AS $function$
    SELECT DISTINCT sub_category FROM product_file WHERE sub_category IS NOT NULL ORDER BY sub_category
$function$
;
